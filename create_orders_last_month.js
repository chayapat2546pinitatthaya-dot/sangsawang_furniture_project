...
// ฟังก์ชันอัปเดตวันที่และสถานะในฐานข้อมูล
async function updateOrderDateAndStatus(orderId, orderDate, status, pool) {
  try {
    await pool.execute(
      'UPDATE `order` SET order_date = ?, order_status = ? WHERE order_id = ?',
      [orderDate, status, orderId]
    );
    return true;
  } catch (error) {
...
// ฟังก์ชันสร้างวันที่สุ่มในเดือนที่แล้ว
function getRandomDateInLastMonth() {
  const now = new Date();
  const lastMonth = new Date(now.getFullYear(), now.getMonth() - 1, 1);
  const lastDayOfLastMonth = new Date(now.getFullYear(), now.getMonth(), 0);
  
  // สุ่มวันที่ในเดือนที่แล้ว
  const randomDay = Math.floor(Math.random() * lastDayOfLastMonth.getDate()) + 1;
  const randomDate = new Date(lastMonth.getFullYear(), lastMonth.getMonth(), randomDay);
...
async function createOrders() {
  console.log('เริ่มต้นการสร้างคำสั่งซื้อ 30 รายการสำหรับเดือนที่แล้ว...\n');
  ...
  try {
    ...
    for (let i = 0; i < totalOrders; i++) {
      ...
      try {
        ...
        createdOrders.push({
          ...
          date: null, // จะอัปเดตทีหลัง
          status: null // จะอัปเดตทีหลัง
        });
        
        console.log(`✅ [${i + 1}/${totalOrders}] สร้างคำสั่งซื้อสำเร็จ: Order ID ${order.order_id} (${customer.name})`);
        
        successCount++;
      } catch (error) {
        console.log(`❌ [${i + 1}/${totalOrders}] สร้างคำสั่งซื้อไม่สำเร็จ: ${error.message}`);
        failCount++;
      }
    ...
    for (let i = 0; i < createdOrders.length; i++) {
      ...
      try {
        await updateOrderDateAndStatus(order.orderId, randomDate, randomStatus, pool);
        ...
        console.log(`✅ [${i + 1}/${createdOrders.length}] อัปเดต Order ID ${order.orderId}: ${randomDate} - ${statusLabels[randomStatus] || randomStatus}`);
      } catch (error) {
        console.log(`❌ [${i + 1}/${createdOrders.length}] อัปเดต Order ID ${order.orderId} ไม่สำเร็จ: ${error.message}`);
      }
    }
    ...
    Object.entries(statusCounts).forEach(([status, count]) => {
      ...
      console.log(`${statusLabels[status] || status}: ${count} รายการ`);
    });
